...
/**
 * @name           CAIJIXIA
 * @version        $Id: index.php 397 2016-10-20 15:26:14Z qinjinpeng $
...
function cjx_a_replace($arr){
	if(strpos($arr[1], 'http://')!==false){
		return $arr[2];
	}
	return $arr[0];
}
...
class CaiJiXia
{
	...
	function AL(){
		...
		$time = time()-3600;
		$this->db->Execute("me","select typeid,count(id) as c from #@__arctiny t where senddate>$time GROUP BY typeid");
	...
	function FB()
	{
		...
		{
			$cforbid = str_replace('\|', '~~$0~~', $this->GV('cforbid'));
			$out = cjx_explode('|', $cforbid);
	...
	function FU($u,$c)
	{
		...
		$u = str_replace('/./', '/', $u);
		$emptyStr = '';
		$h->SetSource($emptyStr, $u, 'media');
		if(preg_match_all("#\s(src|href)\s*=['\"]*([^'\"\s\>]+)['\"\>\s]#si", $c, $mt)){
			foreach ($mt[2] as $k) {
				if(strpos($k, 'javascript:')!==false || strpos($k, '#')!==false){
					continue;
				}
				$fullk = str_replace('/./', '/', $h->FillUrl($k));
				$c = cjx_str_replace(array('"'.$k.'"',"'".$k."'","=".$k),array('"'.$fullk.'"',"'".$fullk."'","=".$fullk),$c);
			}
		}
		return $c;
	}
	...
	function GW()
	{
		...
		$typeStr = empty($typeid)?"":" AND typeid='$typeid'";
		$al = $this->AL();
		if(empty($al)) $this->MG('tsu');
		$al = " AND typeid IN ($al)";
		if(!$rs = cjxdb('kwkeyword')->where("`isclose`=0 AND type<4 $al $typeStr")->order($this->SR())->fields('`nid`,`typeid`,`keyword`,`type`,`pn`')->find()) $this->MG('nolink');
		if($rs['type']==0){
			$p = $this->DP();
			$pn = ($rs['pn']+1)%($p['x']);
		...
		if($this->ci()==false){
			...
			else
			{
				if($this->GV('avc')){
					...
					if(mt_rand(0, 20)==0 && $wx = cjxdb('kwkeyword')->where("type=4 AND isclose=0")->select()){
						$this->WX($wx);
					}else if(mt_rand(0,1)==1 || false == $this->avc()){
						$this->RB();
					}
				}
			}
	...
	function HD($s,$f='',$t='',$ispic=false)
	{
		...
		if(function_exists('curl_init') && function_exists('curl_setopt') && function_exists('curl_exec')){
			$sinfo = parse_url($s);
			$sf = isset($this->sf)?$this->sf:"http://".$sinfo['host'];
			$ch = curl_init();
			...
			curl_setopt($ch, CURLOPT_REFERER, $sf);
			...
			$httpdown->OpenUrl($s);
			$httpdown->m_httphead['content-type'] = 'text/html';
			$c = $httpdown->GetHtml();
			$httpdown->Close();
	...
	function WX($vo)
	{
		...
		$wxs['type'] = 4;
		foreach ($vo as $wx) {
			list($wxname, $wxhao) = explode(',', $wx['keyword']);
			$row['nid'] = $wx['nid'];
			$row['typeid'] = $wx['typeid'];
			$wxs['data'][$wxhao] = $row;
		}
		$rs = $this->AY($wxs);

		if($rs && $rs['status']==1 && !empty($rs['data'])){
			foreach ($rs['data'] as $wxh => $wxl) {
				foreach ($wxl as $arc) {
					$arc['title'] = $this->CT($arc['title'], 'utf-8', $soft_lang);
					$arc['content'] = $this->CT($arc['content'], 'utf-8', $soft_lang);
					$this->vo['ds'] = 
					$this->vo['gk'] = 
					$this->vo['tt'] = empty($arc['title'])?'':trim(html2text($arc['title']));
					$this->vo['litpic'] = preg_replace('#^~#', '/plus/view.php?', $arc['thumb']);
					$arc['content'] = preg_replace('#src="\~#', 'src="/plus/view.php?', $arc['content']);
					$this->vo['by'] = trim($arc['content']);
					$this->FB();

					if(!cjxdb('kwhash')->where(array('hash'=>$arc['hash']))->find() )
					{
						cjxdb('kwhash')->insert(array('hash'=>$arc['hash'], 'nid'=>$wxs['data'][$wxh]['nid']));
						$ar = array();
						$ar['typeid'] = $wxs['data'][$wxh]['typeid'];
	...
	function AY($r)
	{
		if($r['type']==0 || $r['type']==1){
			...
			$wxhaos = join(',', array_keys($r['data']));
			$api = $this->DC('api');
			$host = $_SERVER['HTTP_HOST'];
			$api .= "/Wxcaiji/get?host={$host}&wxhao=".$wxhaos;
			$data = $this->HD($api, false, false, true);
	...
	function FY($bodyrule, $fyrule, $origbody, $origurl, $charset){
		...
		while (list($url, $body) = each($urls)) {
			...
			foreach($dhtml->Links as $l){
				if(strpos($l['link'], '#')!==false || strpos($l['link'], 'index.html')!==false){
					continue;
				}
	...
	function LL($k)
	{
		...
		if($k['type']==0){
			...
			//fix sogou hint_news
			$c = preg_replace('#<table id="hint_container.*?$#si', '', $c);
	...
	function LP($r)
	{
		...
		$r = preg_replace_callback('/<a([^>]*)>(.*)<\/a>/isU', 'cjx_a_replace', $r);
	...
	function ci(){
		$iy = cjxdb('kwpic')->limit(1)->order('id asc')->select();//单次单张
		...
		$multi_site = $this->GV('multi_site');
		$bhost = $multi_site=='Y'?$this->GV('basehost'):'';
		foreach($iy as $k=>$v)
		{
			...
			$fileurl = $bhost.$imgUrl.'/'.$msN.'_'.$k.$extension;
			$imgdata = $this->HD($v['image'],false,false,true);
	...
	function wxp($avg){
		...
		if(preg_match('#.{20,}\!.{80,}#', $avg)){
			header('Content-type: image/gif');
			$pos = strpos($avg, '!');
			$p = substr($avg, 0, $pos);
			$e = substr($avg, $pos+1);
			$url = base64_decode($p);
			$f = DEDEROOT.$cfg_medias_dir.'/weixin/';
			$e5 = md5($e);
			$f .= substr($e5, 0, 2).'/';
			$f .= substr($e5, 2).'.jpg';
			if(is_file($f)){
				...
				$this->sf('http://mp.weixin.qq.com/');
				$data = $this->HD($url.$e, false, false, true);
				if($data){
					if(!is_dir(dirname($f))){
	...
	function OK()
	{
		...
		{
			$api = $this->DC('api');
			if($this->HD($api.'/?m=check&a=islc&domain='.$_SERVER['HTTP_HOST'])=='yes'){
	...
	function RB()
	{
		...
		if($rs = $this->db->GetOne("SELECT c.id,c.nid,c.url,k.typeid,k.keyword,k.type FROM #@__kwcache c LEFT JOIN #@__kwkeyword k ON c.nid=k.nid WHERE k.isclose=0 $al $where"))
		{
			if($this->MC()){
	...
	function RL()
	{
		$w = $this->GW();
		if($w['type']==3){
			$data = cjxdb('co_htmls')->Fields('aid,url')->where(array('nid'=>$w['keyword'],'isdown'=>0,'isexport'=>0))->select();
			$ar = array();
			foreach($data as $_r){
				if(!cjxdb('kwhash')->where(array('hash'=>md5($_r['url'])))->find() && !$this->FL($_r['url']) )
				{
					cjxdb('kwhash')->insert(array('hash'=>md5($_r['url']), 'nid'=>$w['nid']));
					$ar[] = $_r['aid'];
				}
			}
		}else{
			...
			foreach($ar as $v)
			{
				...
				if($this->GV('isadm')==1 || $w['type']==3){
					...
					if(!cjxdb('kwhash')->where(array('hash'=>md5($v)))->find() && !$this->FL($v) )
					{
						$n++;
						cjxdb('kwcache')->insert(array('nid'=>$w['nid'],'url'=>$v));
						cjxdb('kwhash')->insert(array('hash'=>md5($v), 'nid'=>$w['nid']));
					}
				}
			}
		}
	...
	function VL($s)
	{
		if($this->I1($this->GV('autolink')) && $this->I1($this->GV('slink')))
		{
			...
			{
				if(empty($o)) continue;
				$tmp = cjxdb('taglist')->where("`tag`='{$o}'")->limit(50)->select();
				$count = count($tmp);
	...
	function sm(){
		$this->db = $this->GV('db');
		$this->db->Execute("me","SELECT arc.*,tp.typedir,tp.topid,tp.namerule,tp.moresite,tp.siteurl,tp.sitepath FROM #@__archives arc LEFT JOIN `#@__arctype` tp ON tp.id=arc.typeid where arc.arcrank=0 order by arc.id desc limit 2000");
		$nw = time();
		$bs = $this->GV('basehost');
		$wn = $this->GV('webname');
		$t[] = array($bs,'100%',$nw,$wn);
		while($r = $this->db->GetArray('me') ){
			$arcurl = GetFileUrl($r['id'], $r['typeid'], $r['senddate'], $r['title'], $r['ismake'], $r['arcrank'], $r['namerule'], $r['typedir'], 
				$r['money'], $r['filename'], $r['moresite'], $r['siteurl'], 
				$r['sitepath']);
			if(strpos($arcurl,'http://')===false){
				$arcurl = $bs.$arcurl;
			}
			$t[] = array($arcurl,'90%',$r['pubdate'],$r['title']);
		}
		return $t;
	}
	...
	function cjxxml(){
		...
		if($this->GV('ggmap')=='1' && ( !file_exists($xml) || $nw-filemtime($xml)>600 ) ){
			$t = $this->SM();
			$s = $this->DC('xmlheader');
			foreach($t as $k=>$r){
				$s .= '<url><loc>'.$r[0].'</loc><lastmod>'.MyDate('Y-m-d',$r[2]).'</lastmod><changefreq>daily</changefreq><priority>'.($r[1]*0.01).'</priority></url>';
			}
			$s .= $this->DC('xmlfooter');
			if($ch=='gb2312') $s = gb2utf8($s);
			$this->WF($xml,$s);
		}
		if( $this->GV('bdmap')=='1' && ( !file_exists($xml2) || $nw-filemtime($xml2)>600) ){
			if(empty($t)){
				$t = $this->SM();
			}
			$tp = '';
...
if(file_exists($cjx_config))
{
	...
	if($_do=='Plugins.run'){
		$cjx -> run(1);
	}else if($_do=='plus.view'){
		$cjx -> wxp($_SERVER['QUERY_STRING']);
